...
const SupplierPanel: React.FC = React.memo(() => {
  ...
  const handleRefillSubmit = () => {
    ...
    dispatch(
      refillStock({ productId: selectedProductId, amount: refillAmount })
    );
    toast.success(
      selectedProductId === null
        ? `Added ${refillAmount} items to each product`
        : `Added ${refillAmount} items to ${
  ...
  const handlePasswordSubmit = () => {
    if (password === SUPPLIER_PASSWORD) {
      ...
      if (newAttemptCount >= WRONG_PASSWORD_LIMIT) {
        ...
        toast.error(
          `Incorrect password. ${
            WRONG_PASSWORD_LIMIT - newAttemptCount
          } attempts remaining`
        );
      }
    }
    ...
    <div className="supplier-panel">
      <div>
        <button
          ...
          style={{
            opacity:
              cooldownEndTime && Date.now() < cooldownEndTime ? 0.5 : 1.0,
            cursor:
