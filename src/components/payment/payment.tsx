...
const Payment: React.FC<{
  ...
  const { energyConsumption, isSupplierMode } = useSelector(
  ...
  const handlePurchase = async () => {
    ...
    if (energyConsumption + ROBOT_ARM_ENERGY > MAX_ENERGY_CAPACITY) {
      toast.error(
        "Cannot process purchase: System energy limit reached. Please try again in a moment."
      );
      return;
    }
    ...
    if (selectedPaymentMethod === PaymentMethod.CreditCard) {
      ...
      if (insertedMoney < selectedProduct.price) {
        ...
        toast.error(
          `Please insert ${selectedProduct.price - insertedMoney} more units`
        );
        return;
      }
  ...
  const handleRefund = () => {
    if (insertedMoney > 0) {
      toast.success(
        `Refunded ${insertedMoney} units. Please collect your money.`
      );
      dispatch(refundMoney());
    } else {
            ...
            style={{
              background:
                selectedPaymentMethod === PaymentMethod.CreditCard
                  ...
                  : undefined,
              opacity:
                selectedPaymentMethod === PaymentMethod.CreditCard ? 1 : 0.8,
            }}
          >
            💳 Credit Card
          </button>
      ...
      <div className="button-group is-footer">
        ...
        <button
          ...
          disabled={
            ...
            !selectedProduct ||
            (selectedPaymentMethod === PaymentMethod.Cash &&
              insertedMoney < (selectedProduct?.price || 0)) ||
            !selectedPaymentMethod
          }
